...
days = 6
input.data <- read.csv("/Users/Rohandinho/Desktop/Projects/Ara-1_Frequency_Dependence/data/07192012_competition_data.csv")
...
results <- data.frame(Ratio=as.factor(c("9:1","1:1","1:9")),Fitness=c(mean1,mean2,mean3),Left=left.error,Right=right.error)
...
the.plot <- ggplot(results,aes(x=Ratio,y=Fitness)) +
  ...
  labs(title="Difference in Fitness between Competitors") +
  geom_hline(aes(yintercept=1), colour="#990000", linetype="dashed")

ggsave(the.plot, file="/Users/Rohandinho/Desktop/Projects/Ara-1_Frequency_Dependence/results/nuoM_FDplot.pdf")
